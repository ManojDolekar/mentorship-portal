...
function MentorProfile({user}) {
    ...
    const {full_name,qualification,designation,contact_number,email_address,id} =user;
    const navigate=useNavigate();
    const baseUrl='http://localhost:8000/';
    const imagePath=user.file_path
        ...
        <div className=' w-full h-screen flex bg-gradient-to-r from-[#ffd89b] to-[#19547b]  transition-all duration-300 justify-center items-center px-6 py-18 '>
            ...
            className=' p-8 w-full max-w-5xl border-2 border-amber-200 backdrop-blur-2xl  bg-white/40 shadow-2xl  rounded-xl'>
                <div className=' flex flex-row justify-between items-center'>
                    ...
                    <Button type='button' onClick={()=>navigate(`/update_mentor-profile/${id}`)} bgColor='bg-amber-300' className='rounded-lg text-white'>
                        Edit Profile
                    </Button>
                </div>
                <form className=' mt-6'>
                    <div className=' w-full gap-4 grid grid-cols-1 md:grid-cols-2'>
                        ...
                        <div className=' relative w-full h-15 bg-white border-4 border-gray-200 rounded-2xl px-4 py-2'>
                            <h2 className=' absolute top-1 text-gray-700'>Contact Number </h2>
                            <span className='absolute bottom-1 text-md'>{contact_number}</span>
                        </div>
                        <div className=' relative w-full h-15 bg-white border-4 border-gray-200 rounded-2xl px-4 py-2'>
                            <h2 className=' absolute top-1 text-gray-700'>Qualification </h2>
                            <span className='absolute bottom-1 text-md'>{qualification}</span>
                        </div>
                        <div className=' relative w-full h-15 bg-white border-4 border-gray-200 rounded-2xl px-4 py-2'>
                            <h2 className=' absolute top-1 text-gray-700'>Designation </h2>
                            <span className='absolute bottom-1 text-md'>{designation}</span>
                        </div>
                        <div className=' relative w-full h-15 bg-white border-4 border-gray-200 rounded-2xl px-4 py-2'>
                            <h2 className=' absolute top-1 text-gray-700'>Email </h2>
                            <span className='absolute bottom-1 text-md'>{email_address}</span>
                        </div>
                    </div>
                </form>
            </motion.div>
