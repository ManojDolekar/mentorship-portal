...
function AllMenteeDetails() {
    ...
    const{adminStatus}=useSelector(state=>state.auth)
    ...
    const fetchMentees=async()=>{
        try {
            const url=new URL(`http://localhost:8000/api/mentee`)
  ...
  return (
    <Container>
        <div className=' flex  w-full duration-200 bg-gradient-to-r from-[#ffd89b] to-[#19547b] p-4'>
                ...
                <div className=' grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {
                        mentees && (
                            mentees.map((mentee)=>(
                                <UserBar key={mentee.id} userdata={mentee}/> 
                            ))
                        )
                    }
                </div>
...
export default AllMenteeDetails;